...
export default function GapMapModal({ onClose }) {
  ...
  async function handleRun() {
    ...
    try {
      const payload = {
        client: { name: "Gap Map Hub", environment: "hub" },
    ...
    <div className="modal-overlay" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div className="modal modal-wide" role="dialog" aria-modal="true" aria-label="Observability Gap Map">
          ...
          <div className="modal-header-left">
            ...
            <div>
              ...
              <div className="modal-subtitle">
                Visualize service-level blind spots across metrics, logs, traces, dashboards, alerts, and RED
              </div>
            </div>
          </div>
        ...
        <div className="modal-footer">
          ...
          <button className="btn btn-cyan btn-lg" onClick={handleRun} disabled={busy || tools.length === 0}>
            {running ? (
              <>
                <span className="spinner" /> Running Gap Map…
              </>
            ) : (
              `▶ Run Gap Map (${tools.length} tool${tools.length !== 1 ? "s" : ""})`
            )}
          </button>
        </div>
      </div>
