...
def run_observability_gap_map(config_path: Path, output_dir: Path) -> dict[str, str]:
    ...
    logger.info("Extraction complete. Signals=%d dashboards=%d alerts=%d", len(estate.signals), len(estate.dashboards), len(estate.alert_rules))

    result = analyze_observability_gap_map(estate)
